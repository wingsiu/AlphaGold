#!/usr/bin/env python3
"""
Sweep reversal_fvg_long: min_gap (points) × time_from_fvg_bull max (minutes).

Per min_gap: retrain once (gap affects FVG bars + model features).
Per time_max: backtest only via env V14_REVERSAL_FVG_LONG_TIME_FROM_FVG_BULL_MAX.

Combined A/B vs 2398 baseline (4 patterns + reversal_fvg_long).

Usage:
  PYTHONUNBUFFERED=1 .venv/bin/python3 sweep_reversal_fvg_long.py 2025-06-01 2026-05-23
"""
...
PATTERN = "reversal_fvg_long"
TIME_ENV = "V14_REVERSAL_FVG_LONG_TIME_FROM_FVG_BULL_MAX"
GAPS = [0, 1, 3, 5, 10]
TIMES = [15, 20, 30, 45, 60]
TRADES_CSV = PROJECT_ROOT / "runtime" / "v14_pattern_backtest_trades.csv"
OUT_CSV = PROJECT_ROOT / "runtime" / "sweep_reversal_fvg_long.csv"
...
def main() -> None:
    ...
    print(f"\n{'='*72}")
    print(f"  reversal_fvg_long sweep  |  {bt_start} → {bt_end}")
    print(f"  min_gap (pts): {GAPS}  |  time_max (min): {TIMES}")
    print(f"  Env: V14_FVG_MIN_GAP + {TIME_ENV}")
    print(f"{'='*72}\n")
    ...
    if not res.empty:
        best = res.iloc[0]
        print(
            f"\nBest: min_gap={int(best.min_gap)} time<{int(best.time_max)} "
