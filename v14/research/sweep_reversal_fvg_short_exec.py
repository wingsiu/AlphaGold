#!/usr/bin/env python3
"""
Sweep H / TP / SL for reversal_fvg_short with retrain per combo (aligned labels + exec).

Fixed: min_gap=0. Pattern-only SHORT backtest (2398 entry refresh).

Usage:
  PYTHONUNBUFFERED=1 .venv/bin/python3 sweep_reversal_fvg_short_exec.py 2025-06-01 2026-05-23
"""
...
OUT_CSV = PROJECT_ROOT / "runtime" / "sweep_reversal_fvg_short_exec.csv"
...
def main() -> None:
    ...
    time_max = next(r["val"] for r in spec["pattern"] if r["feat"] == "time_from_fvg_bear")
    combos = [(h, tp, sl) for h, tp, sl in product(HORIZONS, TARGETS, STOPS) if sl < tp]
    pfs = pattern_feature_set_for(PATTERN)

    print(f"\n{'='*70}")
    print(f"  reversal_fvg_short H/TP/SL sweep  |  {bt_start} → {bt_end}")
    print(f"  min_gap=0  time_from_fvg_bear<{time_max:.0f}  |  prob≥{PROB_THRESH}")
