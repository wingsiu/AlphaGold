#!/usr/bin/env python3
"""
Sweep H / TP / SL for breakthrough_short with retrain per combo.

Pattern: WR(90) < -70 and ret_3m < -10 (from pattern_features).
Trains walk-forward model for each combo, then backtests SHORT.

Usage:
  python3 sweep_pattern_breakthrough_short_retrain.py
  python3 sweep_pattern_breakthrough_short_retrain.py 2025-06-01 2026-05-23
"""
...
OUT_CSV = PROJECT_ROOT / "runtime" / "sweep_breakthrough_short_retrain.csv"
...
def main() -> None:
    ...
    print(f"\n{'='*70}")
    print(f"  Breakthrough SHORT RETRAIN sweep  |  {bt_start} → {bt_end}")
    print(f"  Gate: WR(90)<-70, ret_3m<-10  |  prob≥{PROB_THRESH}")
    ...
    baseline = res[(res["horizon"] == 30) & (res["tp"] == 20) & (res["sl"] == 15)]
    if not baseline.empty:
        b = baseline.iloc[0]
        print(
            f"Baseline H=30 TP=20 SL=15: {int(b.trades)} trades "
