#!/usr/bin/env python3
"""
Sweep reversal_wick_long: wick_min_pct × time_from_long_lower_wick max (minutes).

Per wick pct: retrain (wick bar definition changes).
Per time_max: combined A/B backtest only (env router override).

Combined: 4-pattern baseline + reversal_wick_long.

Usage:
  PYTHONUNBUFFERED=1 .venv/bin/python3 sweep_reversal_wick_long.py 2025-06-01 2026-05-23
"""
...
TIME_ENV = "V14_REVERSAL_WICK_LONG_TIME_FROM_LONG_LOWER_WICK_MAX"
WICK_PCTS = [0.35, 0.40, 0.45, 0.50]
TIMES = [15, 20, 30, 45, 60]
TRADES_CSV = PROJECT_ROOT / "runtime" / "v14_pattern_backtest_trades.csv"
OUT_CSV = PROJECT_ROOT / "runtime" / "sweep_reversal_wick_long.csv"
...
def main() -> None:
    ...
    print(f"\n{'='*72}")
    print(f"  reversal_wick_long sweep  |  {bt_start} → {bt_end}")
    print(f"  wick_min_pct: {WICK_PCTS}  |  router time_max (min): {TIMES}")
    print(f"  Train: time_from_long_lower_wick < 30  |  Env: {TIME_ENV}")
    print(f"{'='*72}\n")

    base_env = os.environ.copy()
    base_env.pop(TIME_ENV, None)
    base_env.pop("V14_WICK_MIN_PCT", None)
    base_env["V14_FVG_MIN_GAP"] = "0"
    ...
    for pct in WICK_PCTS:
        pct_env = base_env.copy()
        pct_env["V14_WICK_MIN_PCT"] = str(pct)
        print(f"=== wick_min_pct={pct:.0%} — train once ===", flush=True)
        train_pattern(pct_env)

        for tmax in TIMES:
            run_env = pct_env.copy()
    ...
    if not profitable.empty:
        ...
        print(
            f"\nBest profitable: wick_pct={best.wick_min_pct} time<{int(best.time_max)} "
