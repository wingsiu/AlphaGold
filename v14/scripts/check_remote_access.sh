...
if launchctl print "system/com.alphagold.mobile-api" &>/dev/null \
  ...
  echo "      Fix: ./v14/scripts/install_launch_services.sh   OR   --boot-api (not both)"
fi
...
echo ""
echo "Screen Sharing / Remote Desktop is separate — run: ./v14/scripts/check_remote_desktop.sh"
