...
5. After a hang, on the mini: `~/AlphaGold/v14/scripts/fix_remote_desktop.sh`

### Check load during a hang

```bash
~/AlphaGold/v14/scripts/check_remote_desktop.sh
top -o cpu -n 8
```
...
| Script | Purpose |
|--------|---------|
| `v14/scripts/check_remote_desktop.sh` | Is port 5900 open? Legacy VNC? Load warning |
| `v14/scripts/fix_remote_desktop.sh` | Re-enable Screen Sharing (admin password) |
| `v14/scripts/check_remote_access.sh` | Mobile API / launchd (port 8765) |
