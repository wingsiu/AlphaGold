#!/usr/bin/env python3
"""
ML walk-forward on V16 winner pre-close: R=3, struct-exit always, with-trend gate.

Usage:
  PYTHONPATH=. python3 v16/research/momentum_15m_hold_winner_ml.py 2025-06-01 2026-06-25
  PYTHONPATH=. python3 v16/research/momentum_15m_hold_winner_ml.py 2025-06-01 2026-06-25 --preset open
  PYTHONPATH=. python3 v16/research/momentum_15m_hold_winner_ml.py 2025-06-01 2026-06-25 --models logreg lgb gbc
"""
...
def main() -> None:
    ...
    struct_exit = bool(is_cfg.get("exit_on_structure_change", False))

    models = list(ALL_MODELS if args.all_models else (args.models or DEFAULT_MODELS))
    oos_start = pd.Timestamp(args.oos_start, tz="UTC")

    entry_desc = "pre-close≤10 breakout" if args.preset == "preclose" else "open"
    print("=" * 96)
    print(f"  V16 winner ML  |  train {args.train_start}  OOS {args.oos_start} → {args.end}")
    print(f"  preset={args.preset}  entry={entry_desc}  with-trend  R={tp_r}  H={horizon}")
    print(f"  struct_exit={'always' if struct_exit else 'off'}  labels match sim")
    ...
    if "exit_reason" in labeled.columns:
        print(f"  exits: {labeled['exit_reason'].value_counts().to_dict()}")
    ...
    mech = simulate_position_impulse_stop(df_oos, signals_oos, cfg=cfg)
    ...
    out_s = PROJECT_ROOT / f"runtime/v16_winner_{args.preset}_ml_sweep.csv"
    out_b = PROJECT_ROOT / f"runtime/v16_winner_{args.preset}_ml_best.csv"
