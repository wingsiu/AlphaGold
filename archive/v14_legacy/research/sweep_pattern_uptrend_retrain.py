#!/usr/bin/env python3
"""
Sweep H / TP / SL for uptrend_retrace with retrain per combo (aligned labels + exec).

Respects V14_PATTERN_FEATURE_SET (current=96 feats, v2398=93 feats).
Backtest uses 2398 entry refresh (same_dir_refresh=entry, upgrade_stop=False).

Usage:
  python3 sweep_pattern_uptrend_retrain.py
  V14_PATTERN_FEATURE_SET=v2398 python3 sweep_pattern_uptrend_retrain.py 2025-06-01 2026-05-23
"""
...
from config.v14_patterns import PATTERN_REGISTRY, collect_pa_groups, pattern_feature_set
...
OUT_CSV = PROJECT_ROOT / "runtime" / f"sweep_uptrend_retrace_retrain_{pattern_feature_set()}.csv"
...
def main() -> None:
    ...
    rise = next(r["val"] for r in spec["pattern"] if r["feat"] == "rise_from_low_240")
    drop = next(r["val"] for r in spec["pattern"] if r["feat"] == "drop_from_high_240")
    combos = [(h, tp, sl) for h, tp, sl in product(HORIZONS, TARGETS, STOPS) if sl < tp]

    pfs = pattern_feature_set()
    print(f"\n{'='*70}")
    print(f"  Uptrend retrace RETRAIN sweep  |  {bt_start} → {bt_end}")
    print(f"  Definition: rise≥{rise:.0f}  drop≥{drop:.0f}  |  prob≥{PROB_THRESH}")
    print(f"  Feature set: {pfs}")
    print(f"  PA groups: {collect_pa_groups([PATTERN])}")
    print(f"  Combos: {len(combos)} (retrain + backtest each)")
    print(f"  Refresh  : entry TP/H trail (2398 baseline)")
    ...
    print(f"Feature matrix: {len(df_feat)} bars | {n_feat} model features | horizons: {HORIZONS}\n", flush=True)
