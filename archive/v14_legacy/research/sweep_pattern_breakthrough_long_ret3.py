#!/usr/bin/env python3
"""
Sweep breakthrough_long ret_3m rally threshold (WR90 > -30 fixed).

Uses trained model from config exec; varies only the ret_3m gate.
Note: model was trained on pattern bars — after gate change, retrain for production.

Usage:
  python3 sweep_pattern_breakthrough_long_ret3.py 2025-06-01 2026-05-23
"""
...
RET_VALUES = [4, 6, 8, 10, 12, 15, 18, 20, 25]
...
def pattern_mask(full: pd.DataFrame, ret_min: float) -> pd.Series:
    ret_3m = full["ret_3m"] if "ret_3m" in full.columns else full["close"] - full["close"].shift(3)
    return (full["wr_90"] > WR90_MIN) & (ret_3m > ret_min)
...
def simulate_combo(raw, full, prob, ret_min: float) -> dict:
    pat = pattern_mask(full, ret_min)
...
print(f"\n{'='*60}")
print("  Breakthrough LONG — ret_3m rally sweep")
print(f"  WR(90) > {WR90_MIN}  |  ret_3m thresholds: {RET_VALUES}")
...
loose = pattern_mask(full, min(RET_VALUES))
print(f"Scoring with model (loose ret > {min(RET_VALUES)})…")
...
for ret_min in RET_VALUES:
    ...
    r = simulate_combo(raw, full, prob, ret_min)
    r["target_plus"] = tgt_n
    r["target_pct"] = round(tgt_pct, 1)
    rows.append(r)
    print(
        f"{ret_min:6.0f} {r['pattern_bars']:6d} {tgt_n:6d} {tgt_pct:5.1f}% "
        f"{r['signals']:5d} {r['trades']:5d} {r['win_rate']:5.1f}% "
        f"{r['net_pnl']:+8.1f} {r['avg_pnl']:+6.2f}"
    )

out = PROJECT_ROOT / "runtime" / "sweep_breakthrough_long_ret3.csv"
pd.DataFrame(rows).to_csv(out, index=False)
best = max(rows, key=lambda x: x["net_pnl"])
print(f"\nBest PnL: ret_3m > {best['ret_3m_min']:.0f} → {best['trades']} trades, PnL={best['net_pnl']:+.1f}")
print(f"Saved -> {out}")
