...
Config (v28 strict WR90 + v29 retrace/SI):
  WR90 Long      : WR<-80, CV>=15K, Ep>=3, TP=80/SL=30, WF-XGBoost>=0.55
...
Trades on CC.D.CL.BMU.IP (US Crude — same epic as oil_trader / MySQL prices).
"""
...
HEALTH_PATH = PROJECT_ROOT / "runtime" / "oil_bot_health.json"
...
# WR90 Long — imported from oil.signal_engine (LONG_ENTRY, LONG_CV, etc.)
    ...
    while True:
        try:
            ...
            if sec >= MINUTE_OIL_TRADE_START_SEC and minute_key != last_trade_minute:
                ...
                _sync_signal_from_oil()
                ev = evaluate_minute(cached, signal_st, submit=False)
                _sync_oil_from_signal()
                save_oil_state()

                si_result = _decision_log_line(ev.get('si'), 'SI')
                wr_result_str = _decision_log_line(ev.get('wr90'), 'WR90')
                ret_result_str = _decision_log_line(ev.get('ret'), 'Retrace')
                ...
                if flat:
                    for key in ('si', 'wr90', 'ret'):
                        ...
                        if dec and dec.would_enter:
                            submitted = _submit_from_decision(dec, cached, journal)
                            if submitted:
                                _sync_signal_from_oil()
                                if key == 'si':
                                    si_result = f"SI prob={dec.prob:.3f} ENTRY"
                                elif key == 'wr90':
                                    wr_result_str = f"WR90 prob={dec.prob:.3f} ENTRY"
                                else:
                                    ret_result_str = f"Retrace prob={dec.prob:.3f} ENTRY"
