...
Config (v28 strict WR90 + v29 retrace/SI):
  WR90 Long      : WR<-80, CV>=15K, Ep>=3, TP=80/SL=30, XGBoost>=0.55
...
FIX: Save/load XGBoost models to disk (like gold backtest's .joblib files)
     so short windows can load pre-trained models and produce trades.
     Models stored in: oil/wf_models/{wr90|ret|si}/{YYYY-MM}.joblib
"""
...
for _root in (REPO_ROOT, PROJECT_ROOT):
...
from oil.signal_engine import build_15m  # slot-start 15m agg (shared with live)
...
# Model directory (monthly WF .joblib files — same path as live bot)
from oil.wf_ml import OIL_MODEL_DIR, model_path as _model_path

# ======================== DATA LOADING ========================
def load(start='2024-01-01', end='2026-06-30'):
    """Load oil 1m bars from MySQL prices table (full history, no IG stitching)."""
    loader = DataLoader()
...
# Live bot entry priority when multiple legs fire on the same bar.
LEG_ENTRY_PRIORITY = {
    'short_impulse': 0,
    'si': 0,
    'wr90': 1,
...
def merge_single_position(trades: list[dict]) -> list[dict]:
    """One open trade at a time across all legs (matches live IG single slot).

    Skips entries that would open before the prior trade's exit.
    Same-minute tie-break: SI > WR90 > Retrace.
    """
    if not trades:
        return []
...
# ======================== MODEL SAVE/LOAD (like gold backtest) ========================
# _model_path imported from oil.wf_ml


# ======================== ML TRAIN (with disk-persisted models) ========================
def train_ml(d, sigs, tp, sl, feats, stype='ret', ml_th=0.60):
    """Walk-forward XGBoost training with disk-persisted models.

    Like gold backtest: model per month saved to oil/wf_models/{stype}/{YYYY-MM}.joblib
    Short windows load existing models if available.
    Falls back to raw signals if no model exists AND no training data available.
    """
    ...
    for tm in [mo for mo in months if mo >= tstart]:
        ...
        # Try loading saved model for this month first
        saved_path = _model_path(stype, month_str)
        ...
        # Not saved yet — need training data from prior months
        ...
        model_path = _model_path(stype, month_str)
        joblib.dump(model, model_path)
        prib = model.predict_proba(X[tst])[:, 1]
        for j, idx in enumerate(np.where(tst)[0]):
            pr[idx] = prib[j]

    # If no probas were produced at all (no saved model, no training data),
    # fall back: pass all unfiltered trades
    if pr.sum() == 0 and len(p) > 0:
        pr[:] = 1.0  # pass all through threshold
...
def main():
    ...
    print(f'  Models: {OIL_MODEL_DIR} (saved per-month .joblib, like gold backtest)')
    print('=' * 72)
    ...
    # XGBoost filter with disk models (like gold backtest)
    if len(si_recs) > 0:
        ds = pd.DatetimeIndex([r['entry_idx'] for r in si_recs])
        ms = sorted(set(d.to_period('M') for d in ds))
        tsp = pd.Period('2024-07', freq='M')
        X_all = np.array([[float(d1m_s.loc[r['entry_idx']].get(f, 0)) for f in SI_FEATS]
                          for r in si_recs])
        y_all = np.array([1.0 if r['pnl'] > 0 else 0.0 for r in si_recs])
        ...
        for tm in ms:
            ...
            trn = np.array([d.to_period('M') < tm for d in ds])

            if not tst.any():
                continue

            # Try loading saved model
            saved_path = _model_path('si', month_str)
            if saved_path.exists():
                model = joblib.load(saved_path)
            ...
            # Need to train: need at least 20 training rows
            if trn.sum() < 20:
                continue
            if tm < tsp:
                continue

            w = np.where(y_all[trn] == 1)[0]
            l = np.where(y_all[trn] == 0)[0]
            nm = min(len(w), len(l))
            if nm < 5:
                continue
            rng = np.random.RandomState(42 + tm.ordinal)
            bal = np.concatenate([rng.choice(w, nm, 0), rng.choice(l, nm, 0)])
            Xb, yb = X_all[trn][bal], y_all[trn][bal]
            ...
            spath = _model_path('si', month_str)
            joblib.dump(mx, spath)
            prib = mx.predict_proba(X_all[tst])[:, 1]
            for j, idx in enumerate(np.where(tst)[0]):
                sp[idx] = prib[j]
            any_scored = True

        # Fallback: if no model could score anything, pass all records
        if not any_scored and len(si_recs) > 0:
            sp[:] = 1.0
        ...
        for i, r in enumerate(si_recs):
            if sp[i] >= SI_PROB:
                si_tr.append({
                    'entry': r['entry_idx'], 'exit': r['exit_ts'],
        ...
        print(f'  Raw signals: {len(si_sigs)} → records: {len(si_recs)} → '
              ...
              f'WR={sum(1 for x in si_p if x > 0) / len(si_p) * 100:.1f}%') if si_p else print('  SI: 0 after filter')
        all_trades.extend(si_tr)
    else:
        print('  SI: 0 records')

    # ======================== COMBINED (single IG slot) ========================
    if not all_trades:
        print("\nNo trades in period.")
        return

    raw_n = len(all_trades)
    all_trades = merge_single_position(all_trades)
    print(f'\n  Single-slot filter: {raw_n} → {len(all_trades)} trades (no cross-leg overlap)')

    tdf = pd.DataFrame(all_trades)
    tdf['pnl'] = tdf['pnl'].astype(float)
    tdf['entry'] = pd.to_datetime(tdf['entry'], utc=True)
    csv_path = REPO_ROOT / 'runtime' / 'oil_combined_backtest_trades.csv'
