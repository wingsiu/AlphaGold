...
3. **After** models are updated, run a **hybrid backtest without** weak filter, then **rebuild** `runtime/hybrid_weak_time_slots.json` from those trades.
...
V14_WF_TRAIN_MODE=full PYTHONPATH=. .venv/bin/python3 tools/train_patterns.py
