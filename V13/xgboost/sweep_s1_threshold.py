#!/usr/bin/env python3
"""
Sweep script for Stage 1 (v13 Filter) threshold.
Sweeps S1_THRESHOLD from 0.55 to 0.58 in 0.01 increments.
"""
import sys
import os
import pandas as pd
...
def run_threshold_sweep(thresholds):
    ...
    print(f"--- Starting v13 Stage 1 Threshold Sweep ({BACKTEST_START} to {BACKTEST_END}) ---")
    ...
    while current_test_start < end_dt:
        ...
        if chunk_mask.any():
            ...
            # In backtest_v13.py, S2 is only calculated if s1_prob >= threshold.
            # To sweep S1, we should probably pre-calculate S2 everywhere or at least for the lowest threshold?
            # Actually, to be safe and efficient, let's pre-calculate S2 everywhere.
    ...
    for s1_thresh in thresholds:
        print(f"\n--- Testing S1_THRESHOLD = {s1_thresh} ---")
        ...
        if all_trades:
            ...
            temp_csv = PROJECT_ROOT / "xgboost_filter_model" / f"temp_sweep_{s1_thresh}.csv"
    ...
    print("\n" + "="*80)
    print(f"{'S1 Thresh':10s} | {'Trades':6s} | {'PnL':8s} | {'PF':6s} | {'Win%':6s} | {'MaxDD':8s}")
    print("-" * 80)
    for r in results:
        print(f"{r['s1_thresh']:10.2f} | {r['trades']:6d} | {r['pnl']:8.1f} | {r['pf']:6.3f} | {r['win_rate']:6.1f} | {r['max_dd']:8.1f}")
    print("="*80)

if __name__ == "__main__":
    thresholds = [0.55, 0.56, 0.57, 0.58]
    run_threshold_sweep(thresholds)
