...
PREP_CACHE_SCHEMA_VERSION = 4  # bumped: fixed regime labels + dual stage2 binary models
...
class Config:
    ...
    min_15m_drop: float
    min_15m_rise: float
    last_bar_change_min_abs: float
    last_bar_min_volume: float
    ...
    use_stage1_day_session_context: bool
    use_stage2_day_session_context: bool
...
STAGE1_DAY_SESSION_CONTEXT_FEATURE_NAMES = [
    "Dchange_utc2_rel",
    "Dupper_wick_utc2_rel",
    "Dlower_wick_utc2_rel",
    "is_asia_session",
    "asia_session_progress",
    "is_london_session",
    "london_session_progress",
    "is_ny_session",
    "ny_session_progress",
]
...
def _session_flag_and_progress(ts_utc: pd.Timestamp, session_key: str) -> tuple[float, float]:
    spec = SESSION_PERIOD_SPECS[session_key]
    local_ts = pd.Timestamp(ts_utc).tz_convert(spec["timezone"])
    ...
    progress = float(minute_of_day - start_min) / float(duration)
    return 1.0, float(np.clip(progress, 0.0, 1.0))


def _build_stage1_day_session_context_features(
    bars: pd.DataFrame,
    sample_ts: pd.DatetimeIndex,
    curr_close: np.ndarray,
) -> np.ndarray:
    """Build causal day+session context features for stage-1 only."""
    if len(sample_ts) != len(curr_close):
        raise ValueError("stage1 day/session context feature inputs must have matching lengths")

    session_day = (pd.DatetimeIndex(bars.index) + pd.Timedelta(hours=STAGE1_DAY_OHL_OFFSET_HOURS)).floor("D")
    day_open = bars["open"].groupby(session_day).transform("first")
    day_high = bars["high"].groupby(session_day).cummax()
    day_low = bars["low"].groupby(session_day).cummin()
    day_frame = pd.DataFrame({"Dopen": day_open, "Dhigh": day_high, "Dlow": day_low}, index=bars.index)
    aligned = day_frame.reindex(sample_ts)
    if aligned.isna().any().any():
        raise ValueError("Could not align UTC+2 stage1 day/session context features to sample timestamps")

    ref = np.asarray(curr_close, dtype=np.float64)
    dopen = aligned["Dopen"].to_numpy(dtype=np.float64)
    dhigh = aligned["Dhigh"].to_numpy(dtype=np.float64)
    dlow = aligned["Dlow"].to_numpy(dtype=np.float64)
    drange = dhigh - dlow
    drange_safe = np.where(np.abs(drange) < 1e-9, 1.0, drange)

    dchange = (ref - dopen) / drange_safe
    upper_wick = np.maximum(dhigh - np.maximum(dopen, ref), 0.0) / drange_safe
    lower_wick = np.maximum(np.minimum(dopen, ref) - dlow, 0.0) / drange_safe

    ts_idx = pd.DatetimeIndex(sample_ts)
    asia = np.zeros((len(ts_idx), 2), dtype=np.float64)
    london = np.zeros((len(ts_idx), 2), dtype=np.float64)
    ny = np.zeros((len(ts_idx), 2), dtype=np.float64)
    for i, ts in enumerate(ts_idx):
        asia[i, :] = _session_flag_and_progress(ts, "hkt")
        london[i, :] = _session_flag_and_progress(ts, "london")
        ny[i, :] = _session_flag_and_progress(ts, "ny")
...
def _merge_stage1_extras(*extras: Optional[np.ndarray]) -> Optional[np.ndarray]:
    blocks = [np.asarray(e, dtype=np.float64) for e in extras if e is not None]
    if not blocks:
        return None
    if len(blocks) == 1:
        return blocks[0]
    n_rows = blocks[0].shape[0]
    if any(b.shape[0] != n_rows for b in blocks):
        raise ValueError("Stage1 extra feature blocks must have identical row count")
...
def _dataset_cache_key_payload(
    ...
    return {
        ...
        "last_bar_change_min_abs": float(cfg.last_bar_change_min_abs),
        "last_bar_min_volume": float(cfg.last_bar_min_volume),
        ...
        "regime_label_horizon_bars": int(cfg.horizon),
        "regime_label_move_abs": float(REGIME_LABEL_MOVE_ABS),
...
def _load_or_build_supervised_dataset(
    ...
    cache_root = _resolve_prep_cache_dir(cfg.prep_cache_dir)
    if cache_root is None:
        X_tensor, y, stage1_trend_a, ts, curr_a, entry_a, fut_a, entry_ts_a, fut_ts_a, kept, skipped = build_dataset(
            bars,
            cfg.window,
            min_window_range,
            cfg.horizon,
            ...
            last_bar_change_min_abs=cfg.last_bar_change_min_abs,
            last_bar_min_volume=cfg.last_bar_min_volume,
        ...
        return X_tensor, y, stage1_trend_a, ts, curr_a, entry_a, fut_a, entry_ts_a, fut_ts_a, kept, skipped, {
            "enabled": False,
            "kind": "supervised_dataset",
        }
    ...
    if cache_path.exists() and not cfg.refresh_prep_cache:
        ...
        return (
            data["X_tensor"],
            data["y"],
            data.get("stage1_trend_a", np.isin(data["y"], TREND_LABELS).astype(np.int64)),
            data["ts"],
            data["curr_a"],
            data["entry_a"],
            data["fut_a"],
    ...
    return X_tensor, y, stage1_trend_a, ts, curr_a, entry_a, fut_a, entry_ts_a, fut_ts_a, kept, skipped, status
...
_REGIME_MIN_BARS = 30
REGIME_LABEL_HORIZON_BARS = 15
REGIME_LABEL_MOVE_ABS = 10.0


def _compute_confirmed_regime_state(
    ma7_a: np.ndarray,
    ma25_a: np.ndarray,
    min_bars: int,
) -> tuple[np.ndarray, np.ndarray]:
    """Build per-bar regime state and regime-end index from MA relations.

    state values: -1=down, 0=flat, 1=up.
    A regime becomes active only after the MA relation persists for min_bars.
    """
    n = int(len(ma7_a))
    state = np.zeros(n, dtype=np.int8)
    end_idx = np.full(n, -1, dtype=np.int32)

    rel = np.zeros(n, dtype=np.int8)
    valid = (~np.isnan(ma7_a)) & (~np.isnan(ma25_a))
    rel[valid & (ma7_a > ma25_a)] = 1
    rel[valid & (ma7_a < ma25_a)] = -1
    ...
    while i < n:
        ...
        start = i
        i += 1
        while i < n and int(rel[i]) == sgn:
            i += 1
        end = i - 1
        run_len = end - start + 1
        if run_len >= int(min_bars):
            confirmed_start = start + int(min_bars) - 1
            state[confirmed_start : end + 1] = sgn
            end_idx[confirmed_start : end + 1] = end

    return state, end_idx


def _regime_hits_from_index(
    regime_state: np.ndarray,
    regime_end_idx: np.ndarray,
    i: int,
    lookahead_bars: int,
) -> tuple[bool, bool]:
    """Apply per-bar regime targeting rules from current regime state.

    1) When current regime is flat: if regime changes within lookahead, target that regime.
    2) When current regime is up/down: if regime ends within lookahead, target next regime.
    """
    n = int(len(regime_state))
    upper = min(n - 1, i + int(lookahead_bars))
    if i >= n - 1 or upper <= i:
        return False, False

    curr = int(regime_state[i])

    if curr == 0:
        for j in range(i + 1, upper + 1):
            nxt = int(regime_state[j])
            if nxt == 1:
                return True, False
            if nxt == -1:
                return False, True
    ...
    end_i = int(regime_end_idx[i])
    if end_i < i:
        return False, False
    # If current regime extends beyond lookahead, target current regime.
    if (end_i - i) > int(lookahead_bars):
        return (curr == 1), (curr == -1)

    for j in range(end_i + 1, upper + 1):
        nxt = int(regime_state[j])
        if nxt == 1:
            return True, False
        if nxt == -1:
...
def _label_original_directional(
    curr_close: float,
    future_high: float,
    future_low: float,
    threshold: float,
    ...
    """Original final target for Stage-2: down / flat / up from move+adverse checks."""
...
def _label_from_regime_hits(up_hit: bool, down_hit: bool) -> int:
...
def _stage1_trend_regime_label_from_hits(up_hit: bool, down_hit: bool) -> int:
    return 1 if (up_hit or down_hit) else 0


def _regime_move_hits(
    curr_close: float,
    future_high: float,
    future_low: float,
    move_abs: float = REGIME_LABEL_MOVE_ABS,
) -> tuple[bool, bool]:
    up_hit = (future_high - curr_close) > float(move_abs)
    down_hit = (curr_close - future_low) > float(move_abs)
    return up_hit, down_hit
...
def build_dataset(
    ...
    last_bar_change_min_abs: float = 0.0,
    last_bar_min_volume: float = 200.0,
    ...
    skipped = 0
    regime_lookahead_bars = int(horizon)
    max_future_needed = max(int(horizon), regime_lookahead_bars)
    total_iters = len(df) - max_future_needed - (window - 1)
    t_ds = time.time()
    ...
    for idx_i, i in enumerate(range(window - 1, len(df) - max_future_needed)):
        ...
        # ── last-bar absolute body-change filter ──────────────────────────────
        # Keep only samples where the latest candle body is sufficiently large:
        #   close-open > +T  OR  close-open < -T.
        if last_bar_change_min_abs > 0.0:
            last_bar_change = float(w["close"].iloc[-1] - w["open"].iloc[-1])
            if abs(last_bar_change) <= float(last_bar_change_min_abs):
                skipped += 1
                continue

        # ── last-bar minimum volume filter ───────────────────────────────────
        if last_bar_min_volume > 0.0:
            last_bar_volume = float(w["volume"].iloc[-1])
            if last_bar_volume <= float(last_bar_min_volume):
                skipped += 1
                continue
        ...
        fut_w        = df.iloc[i + 1 : i + regime_lookahead_bars + 1]
        ...
        up_hit, down_hit = _regime_move_hits(curr_close, future_high, future_low, REGIME_LABEL_MOVE_ABS)
        labels.append(_label_from_regime_hits(up_hit, down_hit))
        stage1_trend_labels.append(_stage1_trend_regime_label_from_hits(up_hit, down_hit))
        ts.append(signal_ts)
    ...
    y_stage1 = np.asarray(stage1_trend_labels, dtype=np.int64)
    ...
    return X, y, y_stage1, t, curr_a, entry_a, fut_a, entry_t, fut_t, int(len(y)), skipped
...
def _compute_state_features_from_pred(
    ...
    for i in range(n):
        ...
        if pos != 0 and entry > 0.0:
            ...
            _thr_v = float(long_thr if pos > 0 else short_thr)
            target_abs = _thr_v if _thr_v >= 1.0 else abs(entry) * _thr_v
...
def _build_oof_state_features(
    X_tr: np.ndarray,
    y_tr: np.ndarray,
    stage1_trend_tr: Optional[np.ndarray],
    curr_tr: np.ndarray,
    entry_tr: np.ndarray,
    cfg: Config,
    n_1m_feats: int,
    ...
    stage2_extra_tr: Optional[np.ndarray] = None,
) -> np.ndarray:
    ...
    for fold_i, (fit_idx, val_idx) in enumerate(tscv.split(X_tr), 1):
        ...
        try:
            m1_f, m2_f = train_two_stage(
                ...
                stage1_trend_mask=None if stage1_trend_tr is None else stage1_trend_tr[fit_idx],
                stage1_extra=None if stage1_extra_tr is None else stage1_extra_tr[fit_idx],
                stage2_extra=None if stage2_extra_tr is None else stage2_extra_tr[fit_idx],
            )
            pred_val = predict_two_stage(
                ...
                stage2_extra=None if stage2_extra_tr is None else stage2_extra_tr[val_idx],
            )
...
class ConstantBinaryClassifier:
    """Fallback binary classifier used when a target has a single class."""

    def __init__(self, positive: bool) -> None:
        self.positive = bool(positive)
        self.classes_ = np.array([0, 1], dtype=np.int64)

    def fit(self, X: np.ndarray, y: np.ndarray, sample_weight: Optional[np.ndarray] = None) -> "ConstantBinaryClassifier":
        _ = (X, y, sample_weight)
        return self

    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        p1 = 1.0 if self.positive else 0.0
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        return np.full(len(X), 1 if self.positive else 0, dtype=np.int64)
...
def _fit_binary_model(
    ...
    classes = np.unique(y)
    if classes.size == 1:
        return ConstantBinaryClassifier(positive=bool(classes[0] == 1)).fit(X, y, sample_weight=sample_weight)
...
def train_two_stage(
    ...
    stage1_trend_mask: Optional[np.ndarray] = None,
    stage1_extra: Optional[np.ndarray] = None,
    stage2_extra: Optional[np.ndarray] = None,
) -> tuple:
    """
    ...
    Stage 2: two independent binaries on trend-only samples:
      - up   : up(1) vs not-up(0)
      - down : down(1) vs not-down(0)
    ...
    if stage2_extra is not None and two_branch and n_1m_feats > 0:
        raise ValueError("Stage-2 extra context features are not supported with --two-branch modes")
    ...
    # ── Stage 1 ───────────────────────────────────────────────────────────────
    if stage1_trend_mask is None:
        trend_mask = np.isin(y, TREND_LABELS)
    else:
        trend_mask = np.asarray(stage1_trend_mask).astype(bool)
        if trend_mask.shape[0] != y.shape[0]:
            raise ValueError("stage1_trend_mask length must match y length")
    ...
    X_s2 = X[trend_mask]
    if stage2_extra is not None:
        X_s2 = _augment_stage1_input(X_s2, np.asarray(stage2_extra)[trend_mask])
    y_s2_up = (y[trend_mask] == LABEL_UP).astype(np.int64)
    y_s2_down = (y[trend_mask] == LABEL_DOWN).astype(np.int64)
    sw_s2_up = compute_sample_weight("balanced", y_s2_up) if np.unique(y_s2_up).size > 1 else None
    sw_s2_down = compute_sample_weight("balanced", y_s2_down) if np.unique(y_s2_down).size > 1 else None

    m2_up = _fit_binary_model(_new_s2(), X_s2, y_s2_up, sw_s2_up)
    m2_down = _fit_binary_model(_new_s2(), X_s2, y_s2_down, sw_s2_down)

    return m1, {"up": m2_up, "down": m2_down}
...
def predict_two_stage_details(
    ...
    stage2_conflict = np.zeros(len(X), dtype=np.int64)
    ...
    if idx_trend < 0:
        return {
            ...
            "stage2_conflict": stage2_conflict,
        }
    ...
    if trend_mask.any():
        x2 = X[trend_mask]
        if stage2_extra is not None:
            x2 = _augment_stage1_input(x2, np.asarray(stage2_extra)[trend_mask])
        ...
        if isinstance(m2, dict) and "up" in m2 and "down" in m2:
            m2_up = m2["up"]
            m2_down = m2["down"]

            p2_up = m2_up.predict_proba(x2)
            c2_up = list(getattr(m2_up, "classes_", [0, 1]))
            idx_up_pos = c2_up.index(1) if 1 in c2_up else -1
            up_prob = p2_up[:, idx_up_pos] if idx_up_pos >= 0 else np.zeros(len(x2), dtype=np.float64)

            p2_down = m2_down.predict_proba(x2)
            c2_down = list(getattr(m2_down, "classes_", [0, 1]))
            idx_down_pos = c2_down.index(1) if 1 in c2_down else -1
            down_prob = p2_down[:, idx_down_pos] if idx_down_pos >= 0 else np.zeros(len(x2), dtype=np.float64)

            up_signal = up_prob >= up_gate
            down_signal = down_prob >= down_gate

            if hasattr(m2_up, "predict_proba_branches") and idx_up_pos >= 0:
                p2u_1m, p2u_15m = m2_up.predict_proba_branches(x2)
                ...
                up_signal &= p2u_1m[:, idx_up_pos] >= g1m
                up_signal &= p2u_15m[:, idx_up_pos] >= g15

            if hasattr(m2_down, "predict_proba_branches") and idx_down_pos >= 0:
                p2d_1m, p2d_15m = m2_down.predict_proba_branches(x2)
                g1m = stage2_min_prob if stage2_min_prob_1m is None else stage2_min_prob_1m
                g15 = stage2_min_prob if stage2_min_prob_15m is None else stage2_min_prob_15m
                down_signal &= p2d_1m[:, idx_down_pos] >= g1m
                down_signal &= p2d_15m[:, idx_down_pos] >= g15

            up_prob_full[idx_global] = up_prob
            only_up = up_signal & (~down_signal)
            only_down = down_signal & (~up_signal)
            stage2_conflict[idx_global[up_signal & down_signal]] = 1
            result[idx_global[only_up]] = LABEL_UP
            result[idx_global[only_down]] = LABEL_DOWN
            signal_prob[idx_global[only_up]] = up_prob[only_up]
            signal_prob[idx_global[only_down]] = down_prob[only_down]
        else:
            # Backward compatibility for saved bundles that contain a single stage-2 model.
    ...
    *,
    ...
    for i in range(n):
        ...
        if pos != 0 and entry_px > 0.0:
            ...
            _thr_v = float(long_thr if pos > 0 else short_thr)
            target_abs = _thr_v if _thr_v >= 1.0 else abs(entry_px) * _thr_v
            stop_abs = float(long_stop if pos > 0 else short_stop)
            if move_abs >= target_abs:
                pos = 0
                entry_px = 0.0
...
def optimize_min_range(
    ...
    use_stage1_day_session_context: bool = False,
) -> float:
    ...
    for combo_i, rng_val in enumerate(_OPT_RANGES, 1):
        ...
        try:
            X, y, stage1_trend_a, ts, curr_a, entry_a, fut_a, entry_ts_a, fut_ts_a, kept, skipped = build_dataset(
                ...
                last_bar_change_min_abs=last_bar_change_min_abs,
                last_bar_min_volume=last_bar_min_volume,
        ...
        stage1_day_ohl = _build_stage1_day_ohl_features(bars, ts, curr_a) if use_stage1_day_ohl_utc2 else None
        stage1_day_session = (
            _build_stage1_day_session_context_features(bars, ts, curr_a)
            if use_stage1_day_session_context else None
        )
        stage1_extra = _merge_stage1_extras(stage1_day_ohl, stage1_day_session)
        try:
            m1, m2 = train_two_stage(
                ...
                stage1_trend_mask=stage1_trend_a[:sp],
...
def optimize_prob_thresholds(
    ...
    stage2_extra_val: Optional[np.ndarray] = None,
) -> tuple[float, float]:
...
def select_wf_cycle_config(
    ...
    stage1_trend_fit = None if stage1_trend_tr is None else stage1_trend_tr[:split_fit]
    ...
    stage2_extra_fit = None if stage2_extra_tr is None else stage2_extra_tr[:split_fit]
    stage2_extra_val = None if stage2_extra_tr is None else stage2_extra_tr[split_fit:]
    ...
    for flat_ratio in cfg.wf_sweep_flat_ratios:
        try:
            m1_tmp, m2_tmp = train_two_stage(
                ...
                n_1m_feats=n_1m_feats,
                two_branch_stage=cfg.two_branch_stage,
                stage1_trend_mask=stage1_trend_fit,
                stage1_extra=stage1_extra_fit,
                stage2_extra=stage2_extra_fit,
            )
        except ValueError:
            continue
...
def _backtest_trades_df(
    ...
    def _target_abs_for_pred(pred_cls: int, entry_price: float) -> float:
        ...
        # - 0 < threshold < 1 : percent-of-price target (legacy behavior)
        # - threshold >= 1    : absolute price-units target (e.g., $10)
        if thr_val >= 1.0:
            return abs(thr_val)
        return abs(float(entry_price)) * thr_val
    ...
    for i in range(len(pred)):
        ...
        if open_trade is not None and int(pred[i]) not in NO_TRADE_LABELS:
            if int(pred[i]) != int(open_trade["pred"]) and _sig_prob(i) > reverse_exit_prob:
                ...
                if signal_ts < rolled_deadline:
                    ...
                    # Update target only when the rolled target price improves in trade direction.
                    # This keeps dynamic rolling behavior meaningful for both percent and absolute
                    # target modes (e.g. fixed $10 from the latest signal bar).
                    current_target_price = float(open_trade.get("last_target_price", open_trade.get("planned_exit_price", entry_price)))
                    if (new_planned_exit - current_target_price) * side_num > 0.0:
        ...
        if open_trade is not None and signal_ts >= pd.Timestamp(open_trade["planned_exit_time"]):
            ...
            side_num = float(open_trade["side_num"])
            resolved_exit_ts, resolved_exit_px = _market_close_at_or_before(planned_exit_ts, side_num)
            _append_trade(open_trade, resolved_exit_ts, resolved_exit_px, "timeout")
            next_entry_allowed_ts = resolved_exit_ts
            open_trade = None
...
def parse_args() -> Config:
    ...
    p.add_argument("--min-15m-drop",    type=float, default=0.0,
                   help="Keep windows containing a 15-bar downside move >= this value in $ (default: 0, off)")
    ...
    p.add_argument("--label-move-abs", type=float, default=None,
                   help="Override REGIME_LABEL_MOVE_ABS training label threshold (default: 10.0). "
                        "A bar is labeled trend only if price moves >= this amount (absolute $) within horizon bars.")
    p.add_argument("--last-bar-change-min-abs", type=float, default=0.0,
                   help="Keep samples only when abs(last_bar_close - last_bar_open) > this value in $ (default: 0, off)")
    p.add_argument("--last-bar-min-volume", type=float, default=200.0,
                   help="Keep samples only when last bar volume > this value (default: 200, 0=off)")
    ...
    p.add_argument("--use-stage1-day-session-context", action="store_true",
                   help="Append stage-1 day/session context features: day_change, day upper/lower wick, and Asia/London/NY session flags + hour progress.")
    p.add_argument("--use-stage2-day-session-context", action="store_true",
                   help="Append the same day/session context features to Stage-2 directional models.")
    ...
    p.add_argument("--weak-periods-json", default=DEFAULT_WEAK_PERIODS_JSON,
                   help="JSON file listing weak session/day/hour cells to exclude at backtest entry time. Accepts either a top-level weak_cells array or a raw array of {session, day, hour} objects.")
    ...
    args = p.parse_args()
    # Override global label threshold if requested
    if args.label_move_abs is not None:
        global REGIME_LABEL_MOVE_ABS
        REGIME_LABEL_MOVE_ABS = float(args.label_move_abs)
    ...
    return Config(
        ...
        last_bar_change_min_abs=args.last_bar_change_min_abs,
        last_bar_min_volume=args.last_bar_min_volume,
        ...
        use_stage1_day_session_context=args.use_stage1_day_session_context,
        use_stage2_day_session_context=args.use_stage2_day_session_context,
...
def main() -> int:
    ...
    if cfg.last_bar_change_min_abs < 0.0:
        raise ValueError("--last-bar-change-min-abs must be >= 0")
    if cfg.last_bar_min_volume < 0.0:
        raise ValueError("--last-bar-min-volume must be >= 0")
    ...
    _log(f"Using --horizon={horizon} for labels and future-close evaluation")
    ...
    if cfg.optimize:
        min_window_range = optimize_min_range(
            ...
            use_stage1_day_session_context=cfg.use_stage1_day_session_context,
        )
        _log(f"Optimized min_window_range={min_window_range}")

    t = _log("Preparing supervised dataset...")
    X_tensor, y, stage1_trend_a, ts, curr_a, entry_a, fut_a, entry_ts_a, fut_ts_a, kept, skipped, dataset_cache_info = _load_or_build_supervised_dataset(
        bars,
        cfg,
        min_window_range,
        threshold,
    ...
    if cfg.max_samples is not None and kept > cfg.max_samples:
        ...
        y = y[start_idx:]
        stage1_trend_a = stage1_trend_a[start_idx:]
        ts = ts[start_idx:]
    ...
    n_total = len(y)
    stage1_extra_feature_names: list[str] = []
    if cfg.use_stage1_day_ohl_utc2:
        stage1_extra_feature_names.extend(STAGE1_DAY_OHL_FEATURE_NAMES)
    if cfg.use_stage1_day_session_context:
        stage1_extra_feature_names.extend(STAGE1_DAY_SESSION_CONTEXT_FEATURE_NAMES)
    stage1_day_ohl_full = _build_stage1_day_ohl_features(bars, ts, curr_a) if cfg.use_stage1_day_ohl_utc2 else None
    stage1_day_session_full = (
        _build_stage1_day_session_context_features(bars, ts, curr_a)
        if cfg.use_stage1_day_session_context else None
    )
    stage1_extra_full = _merge_stage1_extras(stage1_day_ohl_full, stage1_day_session_full)
    stage2_extra_feature_names: list[str] = []
    if cfg.use_stage2_day_session_context:
        stage2_extra_feature_names.extend(STAGE1_DAY_SESSION_CONTEXT_FEATURE_NAMES)
    stage2_extra_full = (
        _build_stage1_day_session_context_features(bars, ts, curr_a)
        if cfg.use_stage2_day_session_context else None
    )
    ...
    if cfg.eval_mode == "single_split":
        ...
        stage1_trend_tr, stage1_trend_te = stage1_trend_a[:split], stage1_trend_a[split:]
        ts_te = ts[split:]
        ...
        stage1_extra_tr = None if stage1_extra_full is None else stage1_extra_full[:split]
        stage1_extra_te = None if stage1_extra_full is None else stage1_extra_full[split:]
        stage2_extra_tr = None if stage2_extra_full is None else stage2_extra_full[:split]
        stage2_extra_te = None if stage2_extra_full is None else stage2_extra_full[split:]
        ...
        print("Class balance train (down/flat/up):", np.bincount(y_tr[y_tr != LABEL_RISKY], minlength=3).tolist())
        print("Class balance test  (down/flat/up):", np.bincount(y_te[y_te != LABEL_RISKY], minlength=3).tolist())
        print("Stage1 balance train (flat/trend):", np.bincount(stage1_trend_tr.astype(np.int64), minlength=2).tolist())
        print("Stage1 balance test  (flat/trend):", np.bincount(stage1_trend_te.astype(np.int64), minlength=2).tolist())
        ...
        if cfg.use_state_features and reuse_model_path is None:
            ...
            meta_tr = _build_oof_state_features(
                ...
                stage2_extra_tr=stage2_extra_tr,
            )
            m1_base, m2_base = train_two_stage(
                ...
                n_1m_feats=n_1m_feats,
                two_branch_stage=cfg.two_branch_stage,
                stage1_trend_mask=stage1_trend_tr,
                stage1_extra=stage1_extra_tr,
                stage2_extra=stage2_extra_tr,
            )
        ...
        if cfg.optimize_prob:
            ...
            if split_fit < 100 or (len(X_tr) - split_fit) < 50:
                ...
                X_fit, y_fit = X_tr[:split_fit], y_tr[:split_fit]
                stage1_trend_fit = stage1_trend_tr[:split_fit]
                X_val = X_tr[split_fit:]
        ...
        if reuse_model_path is not None:
            ...
            saved_use_stage1_day_ohl = bool(
                bundle.get(
                    "use_stage1_day_ohl_utc2",
                    saved_cfg.get("use_stage1_day_ohl_utc2", False),
                )
            )
            if saved_use_stage1_day_ohl != bool(cfg.use_stage1_day_ohl_utc2):
                raise ValueError(
                    "--model-in feature mismatch: --use-stage1-day-ohl-utc2 must match the saved artifact "
                    f"(saved={saved_use_stage1_day_ohl}, current={bool(cfg.use_stage1_day_ohl_utc2)})"
                )
            saved_use_stage1_day_session = bool(
                bundle.get(
                    "use_stage1_day_session_context",
                    saved_cfg.get("use_stage1_day_session_context", False),
                )
            )
            if saved_use_stage1_day_session != bool(cfg.use_stage1_day_session_context):
                raise ValueError(
                    "--model-in feature mismatch: --use-stage1-day-session-context must match the saved artifact "
                    f"(saved={saved_use_stage1_day_session}, current={bool(cfg.use_stage1_day_session_context)})"
                )
            saved_use_stage2_day_session = bool(
                bundle.get(
                    "use_stage2_day_session_context",
                    saved_cfg.get("use_stage2_day_session_context", False),
                )
            )
            if saved_use_stage2_day_session != bool(cfg.use_stage2_day_session_context):
                raise ValueError(
                    "--model-in feature mismatch: --use-stage2-day-session-context must match the saved artifact "
                    f"(saved={saved_use_stage2_day_session}, current={bool(cfg.use_stage2_day_session_context)})"
                )
            ...
            m2_for_shape = m2["up"] if isinstance(m2, dict) and "up" in m2 else m2
            expected_s2_features = getattr(m2_for_shape, "n_features_in_", None)
            got_s2_features = int(X_te.shape[1]) + state_feat_count + (0 if stage2_extra_te is None else int(stage2_extra_te.shape[1]))
        ...
        for wf in windows:
            ...
            if not cfg.wf_disable_sweep:
                cycle_cfg = select_wf_cycle_config(
                    ...
                    long_adverse_limit,
                    short_adverse_limit,
                    stage1_extra_tr=None if stage1_extra_full is None else stage1_extra_full[tr_sl],
                    stage2_extra_tr=None if stage2_extra_full is None else stage2_extra_full[tr_sl],
                )
                _log(
                    "WF sweep: "
            ...
            stage1_trend_tr_cycle = stage1_trend_a[tr_sl]
            stage1_trend_te_cycle = stage1_trend_a[te_sl]
            stage1_extra_tr_cycle = None if stage1_extra_full is None else stage1_extra_full[tr_sl]
            stage1_extra_te_cycle = None if stage1_extra_full is None else stage1_extra_full[te_sl]
            stage2_extra_tr_cycle = None if stage2_extra_full is None else stage2_extra_full[tr_sl]
            stage2_extra_te_cycle = None if stage2_extra_full is None else stage2_extra_full[te_sl]
            ...
            if cfg.use_state_features:
                ...
                meta_tr_cycle = _build_oof_state_features(
                    ...
                    stage2_extra_tr=stage2_extra_tr_cycle,
                )
                m1_base, m2_base = train_two_stage(
                    X_tr_cycle,
                    y_tr_cycle,
                    ...
                    n_1m_feats=n_1m_feats,
                    two_branch_stage=cfg.two_branch_stage,
                    stage1_trend_mask=stage1_trend_tr_cycle,
                    stage1_extra=stage1_extra_tr_cycle,
                    stage2_extra=stage2_extra_tr_cycle,
                )
                pred_te_base = predict_two_stage(
                    X_te_cycle,
                    m1_base,
                    ...
                    stage2_extra=stage2_extra_te_cycle,
                )
                meta_te_base = _compute_state_features_from_pred(
                    pred_te_base,
                    curr_te_cycle,
                ...
                joblib.dump(
                    {
                        ...
                        "use_stage1_day_ohl_utc2": cfg.use_stage1_day_ohl_utc2,
                        "use_stage1_day_session_context": cfg.use_stage1_day_session_context,
                        "use_stage2_day_session_context": cfg.use_stage2_day_session_context,
                        ...
                        "stage2_extra_feature_names": stage2_extra_feature_names,
        ...
        y_te = y[eval_idx]
        stage1_trend_te = stage1_trend_a[eval_idx]
        ...
        print("Class balance eval  (down/flat/up):", np.bincount(y_te[y_te != LABEL_RISKY], minlength=3).tolist())

    # ── Stage 1 ───────────────────────────────────────────────────────────────
    y_te_s1   = np.asarray(stage1_trend_te).astype(np.int64)
    ...
    stage2_conflict_count = int(np.asarray(pred_meta.get("stage2_conflict", np.zeros(len(y_pred), dtype=np.int64))).sum())
    if trend_mask.sum() > 0:
    ...
    print(f"   Stage-2 dual-signal conflicts -> no-trade: {stage2_conflict_count}")
    ...
    joblib.dump(
        {
            ...
            "regime_definition": {
                ...
                "move_abs": float(REGIME_LABEL_MOVE_ABS),
                "stage1": "trend if up_hit or down_hit",
                "stage2_conflict": "if up and down both pass gates => no trade",
            },
            "label_map":     {0: "down", 1: "flat", 2: "up", 3: "risky"},
            "blocked_utc":   BLOCKED_UTC_WINDOWS,
            "trading_day_cutoff": {
    ...
    report = {
        ...
        "stage2": {
            ...
            "conflict_no_trade_count": int(stage2_conflict_count),
        },
