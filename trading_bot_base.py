...
from config.hybrid_config import FILTER_CONFIG, TARGET_CONFIG, MODEL_CONFIG, ENERGETIC_EXECUTION_CONFIG, WF_CONFIG, TIME_FILTER_CONFIG

EXECUTION_CONFIG = ENERGETIC_EXECUTION_CONFIG
from xgboost_filter_model.time_slot_filter import load_weak_filter, is_blocked_entry, resolve_v14_time_filter_path
...
from xgboost_filter_model.train_filter_v14 import add_v14_daily_features, add_v14_window_features
from xgboost_filter_model.momentum_features import add_momentum_features
...
def prepare_v14_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()

    # 1. Base Feature Engineering (re-uses exactly what the model was trained on)
    df = prepare_base_features(df, move_threshold=TARGET_CONFIG["move_threshold"], er_threshold=TARGET_CONFIG["er_threshold"], future_window=TARGET_CONFIG["horizon"], for_live_inference=True)

    # 2. Add Liquidity Zone Indicators
    df = add_liquidity_indicators(df)
    
    # 3. Add v14 explicit features missing from base
    ...
    pa_group = os.environ.get("V14_PA_GROUP", "").strip().lower()
    if pa_group:
        df = add_price_action_features(df)

    # ── Energetic-bar filter (MUST mirror prepare_data_v14 ordering) ────────
...
class AlphaGoldBaseBot:
    def __init__(self):
        self.logger = self._init_logger()
        self.logger.info("--- AlphaGold v14 Bot Startup ---")
        ...
        if wf_path:
            ...
            if wf_file.exists():
                ...
                self.logger.info(
                    f"Time slot filter ON: {len(self.weak_period_cells)} blocked cells from {wf_file.name}"
                )
            else:
                self.logger.warning(f"Time filter path missing: {wf_path}")

        self._load_models()

    def _load_models(self):
        """Loads the S1 and S2 models from the walk-forward directory."""
        ...
        elapsed_days = max(0, (now - wf_anchor).days)
        ...
        if s1_path.exists():
            self.logger.info(f"Loading S1 cycle model: {s1_path.name}")
            self.s1_model = joblib.load(s1_path)
        else:
            prod_s1 = PROJECT_ROOT / "xgboost_filter_model" / "filter_model_v14_wf.joblib"
            self.logger.info(f"Loading S1 PROD model: {prod_s1.name}")
            self.s1_model = joblib.load(prod_s1)
            
        if s2_path.exists():
            self.logger.info(f"Loading S2 cycle model: {s2_path.name}")
            self.s2_model = joblib.load(s2_path)
        else:
            prod_s2 = PROJECT_ROOT / "xgboost_filter_model" / "directional_model_v14_wf.joblib"
            self.logger.info(f"Loading S2 PROD model: {prod_s2.name}")
            self.s2_model = joblib.load(prod_s2)
        ...
        self.v14_s1_cols = [
            ...
            'near_low_zone', 'recovery_long', 'recovery_short',
            'dist_from_open_rel', 'dist_from_high_rel', 'dist_from_low_rel', 'day_range_so_far_rel',
            'ret_3m', 'vol_3m', 'er_3m', 'ret_15m', 'range_15m', 'ret_60m', 'dist_ma_60m', 'range_60m', 
            'breakout_up_60m', 'breakout_down_60m',
            'dist_daily_poc', 'dist_daily_vwap', 'dist_rolling_poc_4h', 'dynamic_tp', 'dynamic_sl',
            'time_from_15m', 'time_from_max', 'time_from_min',
            'dist_fvg_bull_bottom', 'dist_fvg_bull_top',
            'dist_fvg_bear_top', 'dist_fvg_bear_bottom',
            'time_from_fvg_bull', 'time_from_fvg_bear', 
            'time_from_long_upper_wick', 'time_from_long_lower_wick',
            'time_from_fake_up', 'time_from_fake_down'
        ]
        if os.environ.get("V14_SUDDEN_RISE_A", "").strip() and os.environ.get(
            "V14_SUDDEN_DROP_B", ""
        ).strip():
            self.v14_s1_cols.extend(["sudden_move_3m", "time_from_sudden_move"])
        if os.environ.get("V14_CANDLE_15M", "").strip().lower() in ("1", "true", "yes", "on"):
            self.v14_s1_cols.extend([
                "c15_body_pct", "c15_upper_wick_pct", "c15_lower_wick_pct",
                "c15_close_loc", "c15_bullish",
                "c15_doji", "c15_hammer", "c15_shooting_star", "c15_marubozu",
                "c15_bull_engulf", "c15_bear_engulf", "c15_inside_bar",
            ])
        self.v14_s2_cols = self.v14_s1_cols + [
    ...
    def _init_logger(self):
        l = logging.getLogger("AlphaGoldV14")
        ...
        sh.setFormatter(fmt)
        l.addHandler(sh)
        # File — writes to runtime/trading_bot_v14.log
        log_path = PROJECT_ROOT / "runtime" / "trading_bot_v14.log"
    ...
    def poll(self, store_to_db: bool = False):
        ...
        # 2. Compute features
        df = prepare_v14_features(self.cached_df)

        if len(df) < 1:
            self.logger.warning("Not enough data after feature engineering.")
            return

        # latest_ts is now read AFTER prepare_v14_features so it is the real latest
        ...
        if latest_ts not in raw_df.index:
            ...
            return
        # 4. Model Logic
        s1_p = self.s1_model.predict_proba(df.loc[[latest_ts], self.v14_s1_cols])[0][1]
        ...
        if s1_p >= EXECUTION_CONFIG["s1_threshold"]:
            s2_p = self.s2_model.predict_proba(df.loc[[latest_ts], self.v14_s2_cols])[0][1]
            if s2_p >= dynamic_s2:
                side = 1
        ...
        if side != 0 and store_to_db == False:
            ...
            if is_blocked_entry(latest_ts, self.weak_period_cells):
                self.logger.info(f"Signal {side} blocked by time-slot filter at {latest_ts}")
                return
                ...
                if now.weekday() == 5 and now.hour == 1:
                    ...
                    if should_retrain:
                        ...
                        try:
                            ...
                            # 1. Retrain Stage 1
                            s1_script = PROJECT_ROOT / "xgboost_filter_model" / "train_filter_v14.py"
                            ...
                            if res1.returncode == 0:
                                # 2. Retrain Stage 2
                                s2_script = PROJECT_ROOT / "xgboost_filter_model" / "train_stage2_v14_directional.py"
